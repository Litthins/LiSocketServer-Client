...
//构造函数
Socketfunc::Socketfunc(int port) {
    so_linger.l_onoff = 1;
...
//指令发送函数
int Socketfunc::InstructionSend() {
...
//指令重发函数
int Socketfunc::InstructionReSend(int state) {
...
//图片接收函数
int Socketfunc::ImageReceive(char *id) {
...
//图片重新接收函数
int Socketfunc::ImageReReceive(char *id, time_t calendar) {
    strcat(path, strcat(strcat(strncat(temp, ctime(&calendar), strlen(ctime(&calendar)) - 1), id), ".jpg"));
    printf("image path is: '%s'\n", path);
    if ((fp = fopen(path, "wb")) == NULL) {
        perror("create image file error!");
