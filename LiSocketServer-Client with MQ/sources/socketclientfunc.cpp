...
Socketclientfunc::Socketclientfunc() {}//构造函数为空

//初始化函数
int Socketclientfunc::SocketInit(int port, char *ip) {
...
//指令接收函数
int Socketclientfunc::InstructionReceive() {
...
//图片发送函数
int Socketclientfunc::ImageSend(char *path) {
...
//关闭Socket函数，保留用
int Socketclientfunc::Closesock() {
    close(sockfd);
    return 0;
}